...
async function fetchAPI<T>(endpoint: string, options: QueryOptions = {}): Promise<ApiResponse<T>> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`API request failed: ${response.statusText}. URL: ${url}. Details: ${errorBody}`);
    }
...
export async function getWebsiteInfo(options: QueryOptions = {}): Promise<ApiResponse<Website>> {
  ...
  export async function getRelatedVideos(videoId: string, categorySlug: string, options: QueryOptions = {}): Promise<ApiResponse<Video[]>> {
    const defaultOptions: QueryOptions = {
      filters: {
        id: {
          $ne: videoId,
  ...
  export async function getCategoryVideos(categorySlug: string, options: QueryOptions = {}): Promise<ApiResponse<Video[]>> {
    const defaultOptions: QueryOptions = {
      filters: {
        category: {
          name: {
  ...
  export async function getHomeVideos(categoryIds: number | number[]): Promise<Category[]> {
    ...
    if (Array.isArray(categoryIds)) {
      const promises = categoryIds.map(id => 
        fetchAPI<Category>(`categories/${id}`, options)
      );
